...
getAttribute attr (Element name attrs xmls)
  = if result == Nothing then "" else fromJust result
     where result = lookup attr attrs

getChildren :: String -> XML -> [XML]
getChildren name (Null) = []
getChildren name (Text t) = []
getChildren name e@(Element n attrs xml)
  = if name == n then e : c else c
    where c = concat (map (getChildren name) xml)
getChild :: String -> XML -> XML
getChild name xml
  = head (getChildren name xml)

addChild :: XML -> XML -> XML
-- Pre: the second argument is an Element
addChild new (Element n a xmls)
  = Element n a (xmls ++ [new])

getValue :: XML -> XML
getValue xml = Text (getValue' xml)
...
popAndAdd (first : second : xmls)
  | xmls == [] = [addChild first second]
  | otherwise  = (addChild first second) : xmls
...
parse' ('<' : '/' : rest) stack = parse' (tail rest') (popAndAdd stack)
    where (_, rest') = break (=='>') rest
parse' ('<' : rest) stack = parse' rest'' (Element name attributes [] : stack)
    where (name, rest') = break (\c -> c == '>' || isSpace c) (skipSpace rest)
          (attributes, rest'') = parseAttributes rest'
parse' str stack = parse' rest (addText text stack)
    where (text, rest) = break (=='<') str
...
expandXSL' cxt e@(Element "value-of" _ _)
  = concatMap (expandXSL (Element "value-of" [("select", if rest == [] then [] else tail rest)] [])) (getChildren path cxt)
      where select = getAttribute "select" e
            (path, rest) = break (=='/') select
expandXSL' _ (Text t)
  = [Text t]
--expandXSL' cxt e@(Element "for-each" _ _)
-------------------------------------------------------------------------
