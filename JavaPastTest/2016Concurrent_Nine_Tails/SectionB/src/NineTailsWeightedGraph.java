...
public class NineTailsWeightedGraph {
	...
	private void constructMinimumSpanningTree() {
		...
		while (visited.size() < confCopy.size()) {
			int u = -1, v = -1, c = Integer.MAX_VALUE;
			PriorityQueueInterface<WeightedEdge> q = null;
			for (int i = 1; i <= visited.size(); i++) {
				int currU = visited.get(i);
				PriorityQueueInterface<WeightedEdge> currQ = confCopy.get(currU);
				while (!currQ.isEmpty()) {
					WeightedEdge first = currQ.peek();
					if (!visited.contains(first.parent)) {
						int currC = costs[currU] + currQ.peek().weight;
						if (currC < c) {
							c = currC;
							u = currU;
							v = currQ.peek().parent;
	...
	private class WeightedEdge implements Comparable<WeightedEdge> {
		...
		public WeightedEdge(int c, int p, int w) {
			this.child = c;
			this.parent = p;
			this.weight = w;
		}
