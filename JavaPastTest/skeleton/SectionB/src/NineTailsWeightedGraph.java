...
public class NineTailsWeightedGraph {
	...
	private void constructGraph() {
		...
		for (int i = 0; i < NUM_CONFIGURATIONS; i++) {
			PriorityQueueInterface<WeightedEdge> q = generateParents(i);
			configurations.add(i, q);
		}
	}
	...
	private void constructMinimumSpanningTree() {
		...
		int root = 0;
		PriorityQueueInterface<WeightedEdge> q = confCopy.get(root);
		WeightedEdge e = q.peek();
		while(visited.contains(e.parent)) {
			q.remove();
			e = q.peek();
		}

		
		int cost = costs[e.child] + e.weight;
